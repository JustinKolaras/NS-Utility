...
class Command {
    ...
    fn = async (Msg, Context) => {
        ...
        if (rankId >= 252) {
            return Msg.reply("Invalid rank! You can only game-ban members ranked below **Moderator**.");
        }

        const hasModLogs = await modLogs.findOne({ id: playerId });

        const dataForm = Util.makeLogData("Remote Ban", `**Executor:** ${Msg.member.user.tag} **Reason:** ${reason} **@ ${Util.getDateNow()}**`);
